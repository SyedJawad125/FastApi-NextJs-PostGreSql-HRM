...
# Shared schema (used in POST and PATCH)
class PayslipBase(BaseModel):
    payslip_number: str
    pay_period_start: date
    pay_period_end: date
    ...
    medical_allowance: float = 0.0
    other_allowances: float = 0.0
    gross_salary: float = 0.0  # allowed in payload but will be recalculated

    tax_deduction: float = 0.0
    insurance_deduction: float = 0.0
    loan_deduction: float = 0.0
    other_deductions: float = 0.0
    total_deductions: float = 0.0  # allowed in payload but will be recalculated

    net_salary: float = 0.0  # allowed in payload but will be recalculated
    payment_date: Optional[date] = None
    payment_method: Optional[str] = None
    bank_account: Optional[str] = None
    remarks: Optional[str] = None
...
class PayslipUpdate(BaseModel):
    ...
    medical_allowance: Optional[float]
    other_allowances: Optional[float]
    gross_salary: Optional[float] = 0.0  # Included, recalculated

    tax_deduction: Optional[float]
    insurance_deduction: Optional[float]
    loan_deduction: Optional[float]
    other_deductions: Optional[float]
    total_deductions: Optional[float] = 0.0  # Included, recalculated

    net_salary: Optional[float] = 0.0  # Included, recalculated
    payment_date: Optional[date]
    payment_method: Optional[str]
    bank_account: Optional[str]
    remarks: Optional[str]
    ...
    employee_id: Optional[int]  # ✅ Now included in PATCH
    department_id: Optional[int]
    rank_id: Optional[int]
    approved_by_user_id: Optional[int]
